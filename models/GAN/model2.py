...
def Generator(n_samples,arch,DIM_G,z_len, labels,is_training,image_size, reuse, noise=None):
    with tf.variable_scope("Generator", reuse=reuse) as scope:
        if noise is None:
            noise = tf.random_normal([n_samples, z_len])
        output = x_generators.x_generator1(noise,arch,labels,DIM_G,is_training,image_size, reuse)
        return output
